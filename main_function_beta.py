...
class main_fct():import numpy as np
import pandas as pd
import rasterio
import torch
import os
...
class main_fct():
    ...
    def tensor_creator(path_1,path_2,number_files=None):
        ...
        ##############################
        #Here the value 3 should be changed with data_size if we want the pair fo all the elements
        ##############################
        for i in range (number_files):
            print(f"File number {i+1} of {number_files}")
            ...
            try:
                ...
                n = 0
                tmp_band = 0
                Landsat_band = ["B02", "B03", "B04", "B05"]
                for L_band in Landsat_band:
                    tmp_band = tmp_band+1
                    while n<tmp_band:
                        tmp_j = 0
                        for j in file_1:
            ...
            try:
                ...
                for z in range (nb_path):
                    ...
                    n = 0
                    tmp_band = 0
                    Sentinel_band = ["B2", "B3", "B4", "B8"]
                    for S_band in Sentinel_band:
                        tmp_band = tmp_band + 1
                        while n < tmp_band:
                            for j in file_2:
                                #For Sentinel-2
